			...
			<div class="container" style="margin-left: 8%;margin-top: 15px;">
				...
				<?php
					...
					foreach ($items->rgInventory as $itemRGInventory) {
						...
						if ($tradable == 1) {
							$sql = "SELECT * FROM offers WHERE item = '".$itemID."' ORDER BY ID DESC";
							$result = $conn->query($sql);
							$row = $result->fetch_assoc();
							if($offerStates[$row['state']] == "pending")
							{
								//Show Progressbar
								$buttonText = '<a class="waves-effect btn deposit-item-button" style="width: 100%;background-color: #ba554a !important;margin-top: 15px;"><div class="progress" style="background-color: #303030;margin-top: 15px;"><div class="indeterminate secondary" style="background-color: #FFF;"></div></div></a>';
							}
							elseif($offerStates[$row['state']] == "active")
							{
								//Say Acceppt Here
								$buttonText = '<a class="waves-effect btn deposit-item-button" style="width: 100%;background-color: #ba554a !important;margin-top: 15px;" href="http://steamcommunity.com/tradeoffer/'.$row['tradeofferid'].'">Acceppt Here</a>';
							}
							else
							{
								//Say Deposit
								$buttonText = '<a class="waves-effect btn deposit-item-button" style="width: 100%;background-color: #ba554a !important;margin-top: 15px;">Deposit</a>';
							}
							$marketName = $itemDescription->market_hash_name;
							$iconURL    = $itemDescription->icon_url;
							$type       = $itemDescription->type;
							$condition = str_replace(')', '', explode('(', $marketName)[1]);
							$condition = $condition != '' ? $condition : 'None';
							echo '<div class="col s2" style="width:20%">
								<div class="card-panel" id="'.$itemID.'"> 
									<span class="white-text">'.explode('(', $marketName)[0].'</span>
									<p class="white-text" style="margin-top:-1px;">'.$condition.'</p>
									<img src="https://steamcommunity-a.akamaihd.net/economy/image/'.$itemDescription->icon_url.'" style="width: 200px;height: 150px;">
									'.$buttonText.'
								</div>
							</div>';
						}
